...
export async function generateMetadata({ params }: PageProps<'/leads/[reference]'>) {
 ...
 */
export default async function LeadPage({ params }: PageProps<'/leads/[reference]'>) {
  const { reference } = await params;
          ...
          actions={
            <>
              {can(user.role, 'leads', 'update') ? (
                <Link
                  href={`/leads/${lead.reference}/edit`}
