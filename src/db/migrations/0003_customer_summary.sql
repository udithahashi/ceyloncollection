-- customer_summary: the derived half of a customer.
--
-- The business's customer list asks for nine figures that are not properties of a
-- customer at all - total requests, first and last contact, days since last, latest
-- status, repeat or new, ready-to-buy count, last interest, and a suggested action.
-- Every one is a fact about that customer's leads.
--
-- WHY A VIEW AND NOT COLUMNS
-- Stored as columns they would need updating on every lead insert, edit, status
-- change, soft delete and restore - twenty-odd write paths, each of which can forget
-- one. The first time a counter drifts, the analytics start lying and nothing
-- announces it. Computed on read they cannot drift, at the cost of a few joins over
-- tables that will hold thousands of rows, not millions. When that stops being true
-- the fix is one `create materialized view` and a refresh, with no application
-- change beyond the schedule.
--
-- WHY A VIEW AND NOT A QUERY IN THE APPLICATION
-- The same shape is needed by the customer list, the customer detail page, the
-- dashboard and later the CSV export. Written four times it will be subtly
-- different four times - one of them forgetting `deleted_at is null`.
--
-- WHAT IS DELIBERATELY ABSENT
-- "Days since last contact" and the suggested action. Both are policy, not fact:
-- days-since depends on today's date in Doha, and the action depends on thresholds
-- the business will want to tune ("chase after three days" may become two). Facts
-- live here, policy lives in TypeScript where it can be tested and changed without
-- a migration. See src/features/customers/summary.ts.
